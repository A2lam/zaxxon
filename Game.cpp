...
Game::Game()
	: mWindow(sf::VideoMode(1440, 900), "Zaxxon", sf::Style::Close)
	, mTexture()
	, mPlayer()
	, mFont()
	, mStatisticsText()
	...
	, mIsMovingLeft(false)
{
	...
	_TextureWeapon.loadFromFile("Media/Textures/SI_WeaponPlayer.png");
	_TextureWeaponEnemy.loadFromFile("Media/Textures/SI_WeaponYellow.png");
	_TextureWeaponEnemyMaster.loadFromFile("Media/Textures/SI_WeaponPink.png");
	mTexture.loadFromFile("Media/Textures/30.png");
	_TextureEnemyMaster.loadFromFile("Media/Textures/28.png");
	_TextureEnemy.loadFromFile("Media/Textures/33.png");
...
void Game::InitSprites()
{
	...
	mPlayer.setPosition(50.f, 450.f);
	...
	_EnemyMaster.setPosition(1100.f, 500);
	...
	for (int i = 0; i < SPRITE_COUNT_X; i++)
	{
		for (int j = 0; j < SPRITE_COUNT_Y; j++)
		{
			...
			_Enemy[i][j].setPosition(350.f + 95.f * (i + 1), 105.f + 70.f * (j + 1));
...
void Game::HandleEnemyMasterWeaponFiring()
{
	...
	sw->m_sprite.setPosition(
		x + _TextureEnemyMaster.getSize().x / 2 - 4,
		y + _TextureEnemyMaster.getSize().y / 2);
...
void Game::HandleEnemyMasterMove()
{
	for (std::shared_ptr<Entity> entity : EntityManager::m_Entities)
	{
		...
		if (y >= ((BLOCK_COUNT) * 140) || y <=  80)
		{
			if (entity->m_bTopToBot == true)
			{
				entity->m_bTopToBot = false;
				entity->m_times = 0;
			}
			else
			{
				entity->m_bTopToBot = true;
				entity->m_times = 0;
			}
...
void Game::handlePlayerInput(sf::Keyboard::Key key, bool isPressed)
{
	...
	if (key == sf::Keyboard::Space)
	{
		...
		sw->m_sprite.setPosition(
			EntityManager::GetPlayer()->m_sprite.getPosition().x + EntityManager::GetPlayer()->m_sprite.getTexture()->getSize().x / 2 + 45,
